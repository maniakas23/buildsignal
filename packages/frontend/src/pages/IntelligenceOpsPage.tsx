...
import { Activity, Server, AlertTriangle, Settings, RefreshCw, ArrowRight } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => navigate("/operations-center")} className="gap-2">Operations Center <ArrowRight className="h-4 w-4"/></Button>
        </div>
      </div>

      <div className="grid gap-4 md:grid-cols-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Engine Status</CardTitle></CardHeader><CardContent><div className="flex items-center gap-2"><Server className={`h-5 w-5 ${status?.status === "healthy" ? "text-green-500" : "text-yellow-500"}`}/><span className="font-medium capitalize">{status?.status || "Unknown"}</span></div></CardContent></Card>
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="mt-4"><div className="text-muted-foreground">Intelligence operations overview</div></TabsContent>
