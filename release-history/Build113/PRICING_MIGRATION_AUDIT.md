...
| `packages/frontend/src/pages/PricingRevenuePage.tsx` | Legacy tiers (Starter $49, Pro $149) | Canonical tiers | `Starter`, `Pro`, old prices |
...
|------|----------|------------|
| Historical database records may still contain `starter`/`pro` | Low | Compatibility layer handles migration |
| Old Stripe products with legacy lookup keys | Low | Webhook handler uses `metadata.plan` (canonical) |
| Cached frontend bundles in CDN | Low | New deployments invalidate cache |
| Third-party integrations referencing old plan names | Medium | Documented in compatibility layer |
