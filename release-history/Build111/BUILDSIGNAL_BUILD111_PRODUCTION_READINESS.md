...
| File | Change |
|------|--------|
| `deploy.sh` | Rewritten to 16-gate fail-closed pipeline |
| `scripts/content-scan.js` | Added pricing-tier rule, added skipFiles support |

### Playwright E2E Tests (Task 3)
| File | Change |
|------|--------|
| `packages/frontend/e2e/dashboard.spec.ts` | **NEW** — Tests dashboard, opportunities, watchlist, alerts, reports, billing |
...
### Unit Tests
- `packages/frontend/src/test/pricing.test.ts` — Updated to new pricing tiers
...
**Behavior:** Any gate failure aborts deployment immediately.
...
|------|----------|------------|
| `YOUR_PREVIEW_DATABASE_ID` placeholder not replaced | **Medium** | Must be set before first preview deployment |
| Stripe secrets not configured | **Medium** | Must be set via `wrangler secret put` |
| Playwright tests not run against live environment | **Low** | Tests are written and pass locally |
| No paying customers yet | **None** | By design. All metrics display $0 MRR / 0 customers |
| Kestovar Engine availability | **Low** | BuildSignal gracefully degrades |
| D1 migration history | **Low** | Verify all required migration files |
...
- [x] One pricing model exists everywhere
...
- [x] Preview deployment configuration is complete
